#!/usr/bin/env node
/**
 * sync-orders.js — External script to download orders from Google Sheets
 *
 * Reads all order tabs from the warehouse spreadsheet and writes them
 * to src/data/warehouseOrders.json. Does NOT touch the app — purely
 * a one-way read from Sheets → local JSON.
 *
 * Usage:
 *   cd scripts/google-sheets
 *   npm install
 *   node sync-orders.js              # download all order tabs
 *   node sync-orders.js --dry-run    # preview without writing
 *   node sync-orders.js --tab "204 Cindy 2/28/26"  # download one tab
 */
...
// Tabs to skip (not orders)
const SKIP_TABS = new Set([
  'Forma to copy dont delete',
  'Template',
]);

// ── Product Catalog (inline subset for UPC/price lookups) ───────────
// We dynamically import the catalog from src/data so it stays in sync
let PRODUCT_CATALOG = [];
try {
  const catalogPath = path.join(__dirname, '..', '..', 'src', 'data', 'productCatalog.js');
  const catalogSrc = fs.readFileSync(catalogPath, 'utf-8');
  // Extract the array from the ES module source
  const match = catalogSrc.match(/export const PRODUCT_CATALOG\s*=\s*(\[[\s\S]*?\n\];)/);
  if (match) {
    // Safe eval of the array literal (contains only plain objects)
...
// ── Parse a sheet tab into an order object ──────────────────────────
function parseTabName(tabName) {
  // Format: "204 Cindy 2/28/26" or "206 eastern shore 2/26/26"
...
function parseSheetData(rows, tabMeta) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    const net = parseFloat(row[NET_COL]) || 0;
...
async function main() {
  ...
  // Load existing orders to preserve IDs for tabs we already have
  ...
  for (const tab of orderTabs) {
    ...
    try {
      ...
      // Preserve existing order ID + lineIds if tab was already synced
      const prev = existingByTab[tab];
      if (prev) {
        order.id = prev.id;
        order.createdAt = prev.createdAt;
        // Preserve lineIds for items that match by SKU
        const prevLineIds = {};
  ...
  // Also keep any orders that came from the app (source !== 'sheet-sync')
